...
public class PlaceRemovalEligibilityCommand extends AbstractIncludeHierarchyCommand<InterfacePlaceAction> {
	...
	public Result<InterfacePlaceAction> execute(IncludeHierarchy includeHierarchy) {
		...
		if (targetPlace != null) {
			...
			result.addEntry(buildMessage(place.getId(), place.getId(), includeHierarchy, Reason.NOT_FOUND),
				new RemovePlaceFunctionalExpressionAction(includeHierarchy, place, ""));
		}
		return result; 
	...
	private void buildListOfAffectedArcs(IncludeHierarchy includeHierarchy) {
		...
		if (outboundArcs.size() != 0) { 
			for (OutboundArc arc : outboundArcs) {
				result.addEntry(buildMessage(targetPlace.getId(), arc.getId(), includeHierarchy, Reason.ARC), 
	...
	protected void buildListOfAffectedFunctionalExpressions(IncludeHierarchy includeHierarchy) {
		...
		if (references.size() != 0) { 
			for (String componentId : references) {
				result.addEntry(buildMessage(targetPlace.getId(), componentId, includeHierarchy, Reason.EXPRESSION), 
	...
	private String buildMessage(String placeId, String componentId, IncludeHierarchy includeHierarchy, Reason reason) {
		...
		switch (reason) {
		case ARC: sb.append(reason); sb.append(componentId); break;  
		case EXPRESSION: sb.append(reason); sb.append(componentId); break;
		case NOT_FOUND: sb.append(reason); break;
		}
		return sb.toString();
	}
	private enum Reason {
