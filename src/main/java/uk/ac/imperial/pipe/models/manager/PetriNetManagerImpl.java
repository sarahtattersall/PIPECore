...
public final class PetriNetManagerImpl implements PetriNetManager {
    ...
    public void createFromFile(File file) throws JAXBException, UnparsableException,  PetriNetFileException, IncludeException, FileNotFoundException {
        ...
        switch (xmlFileEnum) {
		case PETRI_NET:
			createSinglePetriNet(file, petriNetIO);
			break;
	...
	protected void createPetriNetsFromIncludeHierarchy(String filePath) throws JAXBException, FileNotFoundException, IncludeException {
		...
		while (it.hasNext()) {
			createPetriNetFromInclude(it.next()); 
		}
	}

	private void createPetriNetFromInclude(IncludeHierarchy include) {
        petriNetNamer.registerIncludeName(include);
        changeSupport.firePropertyChange(NEW_PETRI_NET_MESSAGE, null, include.getPetriNet());
	}


	protected void createSinglePetriNet(File file, PetriNetReader petriNetIO)
