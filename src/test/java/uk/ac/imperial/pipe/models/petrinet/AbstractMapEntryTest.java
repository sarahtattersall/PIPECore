...
public class AbstractMapEntryTest {
	...
	protected void checkIncludeMapEntries(String comment, IncludeHierarchy includes, boolean print, boolean matchName, boolean matchFullyQualifiedName, FQNME... expectedEntries) {
		checkMapEntries(comment,  includes, "includeMap", includes.getIncludeMap(), expectedEntries,  print, matchName, matchFullyQualifiedName);
	}
...
//				printDetails(map, expectedEntries);
				assertEquals("Number of entries does not match for map "+mapName+" in IncludeHierarchy named "+includes.getName(),expectedEntries.length, map.size());
				for (int i = 0; i < expectedEntries.length; i++) {
						...
						if ((expectedEntries[i].key.equals(entry.getKey())) && 
							...
							if (matchName) {
								if (entry.getKey().equals(entry.getValue().getName())) {
									pass = true; 
									if (print) System.out.println("hieararchy "+includes.getName()+" has map "+mapName+" with key "+entry.getKey()+" and include "+entry.getValue().getName());
								}
								else {
									fail("Map entry found for key: "+expectedEntries[i].key+" and specified IncludeHierarchy, but name was required to match key; found: "+expectedEntries[i].include.getName()); 
								}
							}
							else {
								pass = true; 
					...
					if (!pass) {
						fail("No map entry found for key: "+expectedEntries[i].key+" and IncludeHierarchy named: "+expectedEntries[i].include.getName()); 
					}
				}
			}
	private void checkMapEntries(String comment, IncludeHierarchy includes, String mapName,
			Map<String, IncludeHierarchy> map, FQNME[] expectedEntries, boolean print, boolean matchName, boolean matchFullyQualifiedName) {
//				printDetails(map, expectedEntries);
		assertEquals("Number of entries does not match for map "+mapName+" in IncludeHierarchy named "+includes.getName(),expectedEntries.length, map.size());
		for (int i = 0; i < expectedEntries.length; i++) {
			boolean pass = false; 
			for (Entry<String, IncludeHierarchy> entry : map.entrySet()) {
				...
				if ((expectedEntry.key.equals(entry.getKey())) && 
					...
					if (matchName) {
						if (entry.getKey().equals(entry.getValue().getName())) {
							pass = true; 
							if (print) System.out.println("hierarchy "+includes.getName()+" has map "+mapName+" with key "+entry.getKey()+" and include "+entry.getValue().getName());
	...
	protected boolean checkFullyQualifiedName(FQNME expectedEntry, Entry<String, IncludeHierarchy> entry, boolean print,
		...
		if (matchFullyQualifiedName) {
			if (entry.getValue().getFullyQualifiedName().equals(expectedEntry.fullyQualifiedName)) {
				pass = true;
				if (print) System.out.println("include "+entry.getValue().getName()+
						" has fully qualified name "+entry.getValue().getFullyQualifiedName());
			}
			else {
				fail("Include "+entry.getValue().getName()+" expecting fully qualified name "+expectedEntry.fullyQualifiedName+
	...
	protected void printDetails(Map<String, IncludeHierarchy> map,
		...
		for (int i = 0; i < expectedEntries.length; i++) {
			System.out.println(expectedEntries[i].key+" name "+expectedEntries[i].include.getName()+" unique name "+expectedEntries[i].include.getUniqueName());
		}
		System.out.println("actual map: ");
		for (Map.Entry<String, IncludeHierarchy> entry : map.entrySet()) {
			System.out.println("map: "+entry.getKey()+" name "+entry.getValue().getName()+" unique name "+entry.getValue().getUniqueName());
		}
	}
	protected void checkAllIncludesAllMapEntries(boolean print, String comment, ME... expectedEntries) {
		...
		for (int i = 0; i < totalEntries; i++) {
			...
			checkIncludeMapAllEntries(comment+": expecting "+totalEntries+" for "+include.getName(),include, print, childEntries);
		}
	}
	...
	private void checkNextEntryArrays(boolean print, String comment, ME[] parentExpectedEntries, ME[] childExpectedEntries) {
...
//						System.out.println("parent: "+parent.getName()+" child: "+childExpectedEntries[j].include.getName()+" size: "+currentChildren.size());
						...
						currentChildren = checkCurrentChildren(print, comment, childExpectedEntries, 
			...
			currentChildren = checkCurrentChildren(print, comment,
					childExpectedEntries, currentChildren, parent); 
		}
	}

	private void checkNextEntryArrays(boolean print, String comment, FQNME[] parentExpectedEntries, FQNME[] childExpectedEntries) {
