...
//		assertEquals(IncludeHierarchyMapEnum.INCLUDE_ALL.getMap(includes), includes.getIncludeMapAll()); 
	...
	public void addsNewEntry() throws Exception {
		...
		IncludeHierarchyCommand<UpdateResultEnum> addCommand = 
				new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", childInclude); 
	...
	public void existingEntryRenamed() throws Exception {
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", childInclude).execute(includes).hasResult()); 
		checkIncludeMapEntries(includes, false, true, new ME("a", childInclude)); 
		
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", "b", childInclude).execute(includes).hasResult()); 
		checkIncludeMapEntries("entry renamed in map (although internal name doesnt match -- still a)",
				includes, false, false, new ME("b", childInclude)); 
	}
	@Test
	public void sameEntryCanBeAddedMultipleTimes() throws Exception {
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", childInclude).execute(includes).hasResult()); 
		assertFalse("same add done twice is ok", 
				new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", childInclude).execute(includes).hasResult()); 
		checkIncludeMapEntries(includes, false, true, new ME("a", childInclude)); 
	}
	@Test
	public void entryNotAddedIfAliasAlreadyExists() throws Exception {
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", fredInclude).execute(includes).hasResult()); 
		checkIncludeMapEntries(includes, false, true, new ME("a", fredInclude)); 
		
		Result<UpdateResultEnum> result = new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", childInclude).execute(includes); 
		assertTrue(result.hasResult()); 
		assertEquals(UpdateResultEnum.NAME_ALREADY_EXISTS, result.getEntry().value); 
		assertEquals("UpdateMapEntryCommand:  map entry not added to IncludeMap in IncludeHierarchy top because another entry already exists with key: a",
				result.getMessage()); 
	...
	public void entryAddedIfForceRequestedReplacingExistingAlias() throws Exception {
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", fredInclude).execute(includes).hasResult()); 
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", childInclude, true).execute(includes).hasResult()); 
	...
	public void entryNotRenamedIfNewnameAlreadyExists() throws Exception {
		...
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"b", fredInclude).execute(includes).hasResult()); 
		
		checkIncludeMapEntries("a and b entries",includes, false, true, new ME("a", childInclude), new ME("b", fredInclude)); 
		IncludeHierarchyCommand<UpdateResultEnum> renameCommand = 
				new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", "b", childInclude); 
	...
	public void entryRenamedIfForceRequestedReplacingExistingAlias() throws Exception {
		...
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", "b", childInclude, true).execute(includes).hasResult()); 
		checkIncludeMapEntries("only 1 entry; b/fred replaced by b/child when forced",
				includes, false, false, new ME("b", childInclude)); 
	}
	@Test
	public void entryAddedIfRenameRequestedButPreviousNameAndIncludeDontExist() throws Exception {
		...
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"b", "c", fredInclude).execute(includes).hasResult()); 
		checkIncludeMapEntries("requested rename was mistaken, fred now added",
				includes, false, false, new ME("a", childInclude), new ME("c", fredInclude)); 
	}
	@Test
	public void entryNotAddedIfPreviousNameDoesntExistButIncludeExistsUnderAnotherNameAndReturnsResult() throws Exception {
		assertFalse(new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"a", childInclude).execute(includes).hasResult()); 
		assertFalse("nothing under this key",includes.getIncludeMap().containsKey("b")); 
		assertTrue("but include exists",includes.getIncludeMap().containsValue(childInclude)); 
		Result<UpdateResultEnum> result = new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"b", "c", childInclude).execute(includes); 
		assertTrue(result.hasResult()); 
		...
		checkIncludeMapEntries("rename for existing include with wrong name has no effect",
				includes, false, true, new ME("a", childInclude)); 
...
//	TargetHierarchy exists under different key: c.  Not renamed.  Probable logic error.
	...
	public void entryAddedIfForceRequestedWhenPreviousNameDoesntExistButIncludeExistsUnderAnotherNameAndReturnsResult() throws Exception {
		...
		assertFalse("nothing under this key",includes.getIncludeMap().containsKey("b")); 
		assertTrue("but include exists",includes.getIncludeMap().containsValue(childInclude)); 
		Result<UpdateResultEnum> result = new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,"b", "c", childInclude, true).execute(includes); 
		assertFalse(result.hasResult()); 
	...
	public void entryAddedIfForceRequestedWhenPreviousNameIsNullButIncludeExistsUnderAnotherNameAndReturnsResult() throws Exception {
		...
		assertFalse("nothing under this key",includes.getIncludeMap().containsKey("b")); 
		assertTrue("but include exists",includes.getIncludeMap().containsValue(childInclude)); 
		Result<UpdateResultEnum> result = new UpdateMapEntryCommand<IncludeHierarchy>(IncludeHierarchyMapEnum.INCLUDE,null, "c", childInclude, true).execute(includes); 
		assertFalse(result.hasResult()); 
