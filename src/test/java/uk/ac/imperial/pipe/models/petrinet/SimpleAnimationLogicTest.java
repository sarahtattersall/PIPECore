...
public class SimpleAnimationLogicTest {
	...
	@Test
	public void returnsBothImmediateOfAllPrioritiesAndTimedTransitionsAtAllFiringTimes() throws PetriNetComponentException {
		PetriNet petriNet = APetriNet.with(AToken.called("Default").withColor(Color.BLACK)).and(
			APlace.withId("P0").and(1, "Default").tokens()).and(
			ATimedTransition.withId("T0").andDelay(100)).and(
			AnImmediateTransition.withId("T1").andPriority(1)).and(
			AnImmediateTransition.withId("T2").andPriority(2)).and(
			...
			ANormalArc.withSource("P0").andTarget("T2").and("1", "Default").token());
        ...
        assertEquals("all transitions enabled, all returned, even lower priority immediate transitions," +
        		" and even timed transitions, including those that could not fire at current time.",3, transitions.size()); 
	}
	@Test
	public void otherMethodsThrowUnsupportedOperationException() throws Exception {
		...
		try {
			...
			assertEquals("SimpleAnimationLogic does not support this method.  Use PetriNetAnimationLogic instead.", e.getMessage()); 
		}
